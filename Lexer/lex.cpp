...
int Lex::strPrintf(long begin, long end, tokenType t) //解释字符串
{
    char s[200] = "";
    long len = end - begin;
    int temp = -1;
...
void Lex::unaryPrintf(char s, tokenType t) //解释一元运算符
{
    int temp;
    switch (s)
    {
...
bool Lex::isOperator(char ch) //是否为一元运算符
{
    for (int i = 0; i<16; i++)
    {
        if (ch == unaryOP[i])
...
int Lex::comment(char ch, int i) // i=0
{
...
int Lex::number(char ch, int i) // i=1
{
...
int Lex::myOperator(char ch, int i) //i=2
{
    bool isReturn = false;
    while (!feof(source))
    {
...
int Lex::identifier(char ch, int i) //i=3
{
    beginp[i] = ftell(source) - 1;
    long len = 0;
    bool isQuit = false;
...
int Lex::analyze(string fillName)
{
    int states;
    char s[512];
    int i;
