...
sentence* Grammar::makerows(tokenTypes n, tokenTypes t, int cnt)
{
    //s->next=NULL;
...
string Grammar::getmargin(int margin)
{
    string s = "";
...
void Grammar::SplitString(const string& s, string& v, int &c)
{
    char temp;
    istringstream str(s);
    str >> temp;
...
void Grammar::delTree(treeNode *node) {
    if(node == NULL)
        return;
...
void Grammar::generateTree() {
    ...
    grammarTree = nodes[point] = new treeNode;
